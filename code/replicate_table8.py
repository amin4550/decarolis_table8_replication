...
Outputs:
    output/tables/table8_replication.csv
    output/tables/table8_key_coefficients.csv
    output/tables/table4_panel_c_market_summary.csv
...
def main() -> None:
    ...
    summary.to_csv(OUT_TABLES / "table4_panel_c_market_summary.csv")

    table, key = make_table8(market)
    table.to_csv(OUT_TABLES / "table8_replication.csv", index=False)
    key.to_csv(OUT_TABLES / "table8_key_coefficients.csv", index=False)
    print(table.to_string(index=False))
    print("\nSaved outputs to", OUT_TABLES)
